...
class TestPctColor:
    def test_none_is_gray(self):       assert pct_color(None) == "gray"
    def test_low_is_green(self):       assert pct_color(30)   == "#30D158"
    def test_60_is_orange(self):       assert pct_color(60)   == "#FF9500"
    def test_85_is_red(self):          assert pct_color(85)   == "#FF4A4A"
    def test_59_is_green(self):        assert pct_color(59)   == "#30D158"
    def test_84_is_orange(self):       assert pct_color(84)   == "#FF9500"


# ── fmt_k ─────────────────────────────────────────────────────────────────────

class TestFmtK:
    def test_small(self):         assert fmt_k(500)        == "500"
    def test_thousands(self):     assert fmt_k(68_000)     == "68k"
    def test_millions(self):      assert "M" in fmt_k(1_500_000)
    def test_exactly_1000(self):  assert fmt_k(1_000)      == "1k"
    def test_zero(self):          assert fmt_k(0)           == "0"
...
class TestRecommend:
    def test_all_clear(self):
        ico, _ = recommend(30, 30, False);    assert ico == "🟢"

    def test_context_filling(self):
        ico, msg = recommend(30, 70, False);  assert ico == "🟡"; assert "compact" in msg.lower()

    def test_context_after_compact(self):
        ico, msg = recommend(30, 70, True);   assert ico == "🟡"; assert "compaction" in msg.lower()

    def test_session_high(self):
        ico, _ = recommend(80, 30, False);    assert ico == "🟠"

    def test_context_critical(self):
        ico, _ = recommend(30, 90, False);    assert ico == "🔴"

    def test_both_critical(self):
        ico, _ = recommend(90, 80, False);    assert ico == "🔴"

    def test_ceiling(self):
        ico, _ = recommend(97, 50, False);    assert ico == "🚨"

    def test_none_inputs(self):
        ico, _ = recommend(None, None, False); assert ico == "🟢"

    def test_session_60_is_yellow(self):
        ico, _ = recommend(60, 10, False);    assert ico == "🟡"
...
class TestParseLatestSession:
    ...
    def test_cumulative_output_tokens(self, monkeypatch, tmp_path):
        entries = [
            {"type": "assistant", "message": {"model": "claude-sonnet-4-6-20250514",
...
class TestBackoff:
    def test_backoff_written_on_429(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(_mod.urllib.request, "urlopen", lambda *a, **kw: (_ for _ in ()).throw(Fake429()))
    ...
    def test_backoff_respected_in_main_flow(self, tmp_path, monkeypatch):
        monkeypatch.setattr(_mod, "CACHE_PATH", tmp_path / "last_good.json")
        save_cache({
            "data": {"five_hour": {"utilization": 50.0, "resets_at": None},
                     "seven_day": {"utilization": 20.0, "resets_at": None}},
            "ts": "2026-01-01T00:00:00",
            "backoff_until": time.time() + 600,
        })
        called = []
        monkeypatch.setattr(_mod, "fetch_usage",           lambda t: called.append(True) or ({}, None))
    ...
    def test_backoff_clears_after_expiry(self, tmp_path, monkeypatch):
        ...
        called = []
        monkeypatch.setattr(_mod, "fetch_usage",           lambda t: called.append(True) or ({"five_hour": {}}, None))
        monkeypatch.setattr(_mod, "load_token",            lambda: "sk-ant-" + "fake")
        monkeypatch.setattr(_mod, "parse_latest_session",  lambda: None)
        monkeypatch.setattr(_mod, "_print_all",            lambda *a, **kw: None)
    ...
    def test_retry_after_missing_uses_default(self, tmp_path, monkeypatch):
        ...
        class Fake429NoHeader(urllib.error.HTTPError):
            def __init__(self):
                self.code = 429; self.headers = {}
                self.url  = "https://api.anthropic.com"
                self.msg  = "Too Many Requests"; self.fp = None

        monkeypatch.setattr(_mod.urllib.request, "urlopen",
